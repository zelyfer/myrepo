...
#	input = open(r"E:\ChromeDownload\google-python-exercises\google-python-exercises\basic\alice.txt", 'r')
	for line in input:
		words = line.split()
		for word in words:
			word = word.lower()
...
def print_top(filename):
  ...
  # Sort them so the big counts are first using key=get_count() to extract count.
	items = sorted(newdict.items(), key=lambda a: a[1], reverse=True)
